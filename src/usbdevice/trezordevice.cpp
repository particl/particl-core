...
/*
    This function will close (!) the handle, and therefore shouldn't be used in a loop
    where other messages are expected to be read. It will re-open the handle, and
    temporarily probe the device until it sends the awaited msg_type.
    It can not keep a handle on the device or it is unable to unlock.

    You must re-open the handle after this read.
*/
int CTrezorDevice::CloseAndReadWhenUnlocked(uint16_t msg_type_out_awaited, uint16_t& msg_type_out, std::vector<uint8_t>& vec_out) {
    Close();
    ...
    for (;;) {
        ...
        if (0 != Open()) {
            LogPrintf("%s: in for(;;) loop Open() failed, device busy?.\n", __func__);
            continue;
        }

        if (0 != ReadWithTimeoutV1(handle, msg_type_out, vec_out, 100)) {
            Close();
            LogPrintf("%s: in for(;;) loop ReadWithTimeoutV1() failed.\n", __func__);
            continue;
        }
        ...
        || msg_type_out == hw::trezor::messages::MessageType_PassphraseRequest) {
            // TODO: signal Qt wallet that an unlock must happen.
            LogPrintf("%s: PinMatrix or Passphrase request received.\n", __func__);
            Close(); // We can't hold a lock to the device, else unlocking the device will fail.
            continue;
        }

        if (msg_type_out == msg_type_out_awaited) {
            Close();
            break;
        }
...
/*
    This function will wait until the device is unlocked.
    Then open up the device.
*/
int CTrezorDevice::OpenAsUnlocked()
{
    ...
    if (0 != CloseAndReadWhenUnlocked(hw::trezor::messages::MessageType_Success, msg_type_out, vec_out)) {
        return errorN(1, sError, __func__, "CloseAndReadWhenUnlocked() failed.");
    }

    LogPrintf("%s: Device was successfully unlocked.\n", __func__);
    return Open();
};
...
int CTrezorDevice::GetPubKey(const std::vector<uint32_t>& vPath, CPubKey& pk, std::string& sError)
{
    ...
    if (0 != OpenAsUnlocked()) {
        return errorN(1, sError, __func__, "Failed to open unlocked device.");
    }
...
int CTrezorDevice::Unlock(std::string pin, std::string passphraseword, std::string &sError)
{
    if(pin.size() > 0) {
        ...
        if (0 != GenericUnlock(&vec_pin, hw::trezor::messages::MessageType_PinMatrixAck)) {
    ...
    if (passphraseword.size() > 0) {
        ...
        if (0 != GenericUnlock(&vec_pass, hw::trezor::messages::MessageType_PassphraseAck)) {
